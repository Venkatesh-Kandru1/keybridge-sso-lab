...
export function getVisibleApplicationIds(roles = []) {
  return Object.entries(registeredApplications)
    .filter(([, application]) => roles.includes(application.requiredRole))
    .map(([appId]) => appId);
}

export function validateDemoPasswordChange(
  currentPassword,
  expectedPassword,
  newPassword,
  confirmation,
) {
  if (currentPassword !== expectedPassword) return "Current password is incorrect.";
  if (newPassword.length < 10) return "New password must contain at least 10 characters.";
